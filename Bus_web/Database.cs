...
namespace Bus_web
{
    ...
    {
        SqlConnection conn = new SqlConnection(@"Data Source=DESKTOP-UD452FI\SQLEXPRESS;Initial Catalog=Bus_web;Integrated Security= true;");
        ...
        public bool entrybusdata(int bus_id, string bus_name, string from_where, string to_where, string date_of_journey, string dep_time, string arr_time, int avai_seat, int fare)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                //Response.Write("<script>alert('Error: This bus id exists in database');</script>");
                conn.Close();
                throw new Exception("Error: This bus id exists in database");
            }
            else
            {
                ...
                //Response.Write("<script>alert('Information Inserted Successfully');</script>");
                conn.Close();
                return true;
            }
        }
        ...
        public bool deletebusdata(int bus_id)
        {
            ...
            {
                //Response.Write("<script>alert('Error: This bus schedule does not exist in database');</script>");
                conn.Close();
                throw new Exception("Error: This bus schedule does not exist in database");
            }
        }
        ...
        public bool updatebusdata(int bus_id, string bus_name, string from_where, string to_where, string date_of_journey, string dep_time, string arr_time, int avai_seat, int fare)
        {
            ...
            {
                //Response.Write("<script>alert('Error: This bus id does not exist');</script>");
                conn.Close();
                throw new Exception("Error: This bus id does not exist");
            }
